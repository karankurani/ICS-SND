...
public class LDABase implements Serializable {
    ...
    int minTokenCount = 1;
    short numTopics = 10;
    double topicPrior = 1/(double)numTopics;
    ...
    private static String[] STOPWORD_LIST = setStopWordList(System.getProperty("user.dir") + "/data/input/dblp_stopwords_only.txt");
    private static Set<String> STOPWORD_SET = new HashSet<String>(Arrays.asList(STOPWORD_LIST));

    public LDABase(String corpusFile) throws FileNotFoundException {    	    	
        this.articleTexts = readCorpus(corpusFile);
        symbolTable = new MapSymbolTable();
        docTokens = LatentDirichletAllocation.tokenizeDocuments(articleTexts, WORMBASE_TOKENIZER_FACTORY, symbolTable, minTokenCount);
    }
    ...
    private CharSequence[] readCorpus(String yamlFile) throws FileNotFoundException {
            List<CharSequence> articleTextList = new ArrayList<CharSequence>(15000);
    		Yaml yaml = new Yaml();
            InputStream io = new FileInputStream(yamlFile);
            Iterable<Object> corpus = yaml.loadAll(io);
            List<LinkedHashMap<String,String>> seedList = (List<LinkedHashMap<String,String>>) corpus.iterator().next();
            for(LinkedHashMap<String,String> entry : seedList){
            	articleTextList.add(entry.get(":abstractText")+ " " + entry.get(":title") + " " + entry.get(":booktitle"));
            }                        
...
//	= new RegExTokenizerFactory("[\\x2Da-zA-Z0-9]+"); // letter or digit or hyphen (\x2D)
    static final TokenizerFactory BASE_TOKENIZER_FACTORY = new RegExTokenizerFactory("[a-zA-Z0-9]+"); // letter or digit or hyphen (\x2D)
    static final char[] VOWELS = new char[]{'a', 'e', 'i', 'o', 'u', 'y'};
    static final TokenizerFactory WORMBASE_TOKENIZER_FACTORY = wormbaseTokenizerFactory();
